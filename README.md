...
Autorização via header, não por token. O middleware apenasAdmin confia no header role enviado pelo próprio front (req.headers['role']), que qualquer pessoa pode forjar com um curl. O próprio código já comenta isso ("Em produção, isso virá do token JWT") — vale implementar JWT ou sessão de verdade.
Senha do admin em texto puro. fazerLogin compara senha diretamente com o banco (WHERE cpf = ? AND senha = ?), sem hash. O ideal é usar bcrypt para armazenar e comparar senhas.
URL da API fixa no front. http://localhost:3001 está hardcoded em App.js, Form.js e Grid.js. Colocar em uma variável de ambiente (REACT_APP_API_URL) evita precisar editar código para trocar de ambiente (dev/produção).
package.json duplicado. Existe um package.json na raiz e outro dentro de api/, praticamente idênticos — dá para manter só um dos dois.
Arquivo estranho em front/public/package.json. Parece ter sido criado por engano dentro da pasta public (não deveria existir ali) e ainda tem uma dependência com o nome errado (style-components em vez de styled-components). Vale apagar esse arquivo.
Métodos não utilizados. Passageiro.realizarPagamento() e Administrador.validarAlteracaoDeDados() existem nos models mas não são chamados em nenhum controller — o updatePassageiro recria o objeto do zero em vez de usar essas regras. Boa oportunidade para conectar a lógica de domínio de fato às rotas.
DELETE /passageiros/:id na verdade espera um CPF. Funciona, mas o nome do parâmetro (:id) pode confundir; renomear para :cpf deixa a intenção mais clara.
Sem porta configurável. app.listen(3001) está fixo; usar process.env.PORT || 3001 facilita o deploy.
...
Back-end: Node.js, Express, MySQL (mysql2), dotenv, cors Front-end: React, styled-components, axios, react-toastify, react-icons
